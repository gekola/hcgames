...
pub fn rot90(b: [[u32; N]; N]) -> [[u32; N]; N] { flip_h(transpose(b)) }
...
pub fn slide(board: [[u32; N]; N], dir: u8) -> Option<([[u32; N]; N], u32)> {
    ...
    if sp == p { None } else { Some((unprep(sp, dir), pts)) }
}
...
pub fn can_move(board: &[[u32; N]; N]) -> bool {
    for r in 0..N { for c in 0..N {
        ...
        if r + 1 < N && board[r + 1][c] == board[r][c] { return true; }
        if c + 1 < N && board[r][c + 1] == board[r][c] { return true; }
    }}
    false
}

pub fn choose_dir(board: &[[u32; N]; N]) -> Option<u8> {
    let empty = board.iter().flat_map(|r| r.iter()).filter(|&&v| v == 0).count();

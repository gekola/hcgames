...
fn fig_tree() -> Vec<(i32, i32)> {
    ...
    for half in 0..=5i32 {
        row(-half, half, -13 + half, &mut v);
    }
    // Middle layer (rows -7 to 0)
    row(-2, 2, -7, &mut v);
    for half in 0..=7i32 {
        row(-half, half, -6 + half, &mut v);
    }
    // Bottom layer (rows 2 to 12)
    row(-4, 4, 2, &mut v);
    for half in 0..=9i32 {
        row(-half, half, 3 + half, &mut v);
    }
    // Trunk
    for r in 13..=17 {
        row(-2, 2, r, &mut v);
    ...
    while !remaining.is_empty() {
        let mut layer: Vec<(i32, i32)> = remaining
            ...
            .filter(|&pos| {
                all_dirs
                    .iter()
                    .any(|&d| path_clear(pos, d, &remaining, field_w, field_h))
            })
            .collect();
...
fn anti_aligned_pick(pos: (i32, i32), valid: &[Dir], committed: &HashMap<(i32, i32), Dir>) -> Dir {
    if valid.len() == 1 {
        return valid[0];
    }
    ...
    let candidates: Vec<Dir> = valid
        .iter()
        .copied()
        .filter(|&d| nbr_count(d) == min)
        .collect();
